...
def init_db():
    ...
    if not admin:
        ...
        conn.execute('INSERT INTO users (username, password, role) VALUES (?, ?, ?)',
                     ('admin', pw, 'admin'))
    # Migrate: add max_assignees if missing
    try:
...
def api_users():
    conn = get_db()
    users = conn.execute('SELECT id, username, role FROM users WHERE role = ? ORDER BY username', ('user',)).fetchall()
    conn.close()
...
def serialize_task(conn, task, user_id=None):
    d = dict(task)
...
def api_tasks():
    ...
    result = [serialize_task(conn, t, session['user_id']) for t in tasks]
    conn.close()
...
def api_create_task():
    ...
    cursor = conn.execute('''
        INSERT INTO tasks (title, description, status, priority, due_date, max_assignees, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    ''', (
        title,
...
def api_update_task(task_id):
    ...
    conn.execute('''
        UPDATE tasks SET title=?, description=?, status=?, priority=?, due_date=?, max_assignees=?
        WHERE id=?
    ''', (
